...
from sklearn.metrics import (
    average_precision_score, precision_recall_curve, roc_curve, roc_auc_score, confusion_matrix
)
from imblearn.under_sampling import RandomUnderSampler

import Methods_utils.methods as custom

def plot_custom_confusion_matrix(cm, classes, title, cmap, save_path):
    """Plot and save a custom confusion matrix."""
...
def create_confusion_matrix_plot(y_test, predictions, model_name, save_dir, iteration_label):
    """Create and save confusion matrix plot for a model."""
    ...
    save_path = os.path.join(save_dir, f'{iteration_label}_CM_{model_name}.png')
    plot_custom_confusion_matrix(cm, ['Sepsis-free', 'Sepsis'], 
                                f'Confusion Matrix - {model_name}', 
                                cmap=plt.cm.BuPu, save_path=save_path)
...
def ml_model_cm(model_name, X_train, y_train, X_test, y_test, iteration, plot_number, results_dir):
    """Train, evaluate and return model and metrics using the standard methods (no onset data) with confusion matrix."""
    print(f"DEBUG: ml_model_cm called with model_name='{model_name}', iteration='{iteration}'")
    
    if model_name == 'rf':
        out = custom.random_forest
    elif model_name == 'svm':
        out = custom.svm
    elif model_name == 'xgb':
        out = custom.xgboost_clf
    ...
    # Call the method without onset parameters
    results = out(X_train, y_train, X_test, y_test)
    
    # Extract metrics (standard methods return [model, auc, fpr, tpr, auprc, precision, recall])
    model = results[0]
    auroc_model = results[1]
    fpr_model = results[2]
    tpr_model = results[3]
    auprc_model = results[4]
    precision_model = results[5]
    recall_model = results[6]
    
    # Get predictions for confusion matrix
    predictions = model.predict(X_test)
    
    # Create confusion matrix plot using our internal function
    name = f"{model_name.capitalize()}_{iteration}"
    cm = create_confusion_matrix_plot(y_test, predictions, name, results_dir, plot_number)
    
    # Return empty dataframe since we don't have onset data for time-based analysis
    plot_info = pd.DataFrame()
    
    return model, auroc_model, fpr_model, tpr_model, auprc_model, precision_model, recall_model, plot_info
...
def getData(data_path, CV_nr, config):
    """Load and return data, selected features and best-avg model from the specified paths and config (no onset data)."""
    ...
    # No onset data loading since we don't need it
    ...
    return X, y_toSplit, featureSelection_options, best_avg_model

def trainModels_andTest(X, y_toSplit, featureSelection_options, best_avg_model, run_number, n_iters, config, res_dir):
    """Train models and compute/test performance, saving everything to defined output folder (no onset analysis)."""
    print("--------Starting training and testing on hold-out set -------------------------------------------")

    print(f"DEBUG: n_iters parameter received = {n_iters}")
    results_dir = res_dir#f"./Results_iterationPlots_nw_run_{run_number}_noOnset/"
    ...
    # No onset data processing needed
    ...
    for counter_iter in range(n_iters):
        ...
        for idx, features in enumerate(featureSelection_options):
            ...
            model, auroc_model, fpr_model, tpr_model, auprc_model, precision_model, recall_model, plot_info = ml_model_cm(
                best_avg_model, X_train, y_train, X_test, y_test, label, plot_number, results_dir)
            results_dict['Model'].append(best_avg_model) #changed this, because we are interested in the name, not the whole object from sklearn
            ...
            # Note: No violin plot data since we don't have onset times
    ...
    # Save & plot (only AUROC plots, no violin plots since no onset data)
    plotAUROC_trainAndTest(plot_AUROC_df_train.groupby('Count'), plot_AUROC_df_test.groupby('Count'), results_dir)
    # Note: No violin plots for time bins since we don't have onset time data
    # extract class name from best_avg_model for filtering!
...
def wrapAdvancedAnalysis(CV_nr, run_number, number_ofIterations):
    # print(f"DEBUG: wrapAdvancedAnalysis called with number_ofIterations={number_ofIterations}")
    config = load_config()
    
    RES_DIR = config["results_holdOut"]["results_folder_noOnset"]
    os.makedirs(RES_DIR, exist_ok=True)
    
    log_file_path = os.path.join(RES_DIR, "run_log_hold-out_noOnset.txt")
    ...
    X, y_toSplit, featureSelection_options, best_avg_model = getData(data_path, CV_nr, config)
    trainModels_andTest(X, y_toSplit, featureSelection_options, best_avg_model, run_number, number_ofIterations, config, RES_DIR)

if __name__ == "__main__":
    run_number = 11
    wrapAdvancedAnalysis(10, run_number, 20)
